...
- The app is entirely free — every feature is unlocked for everyone, with no Pro tier or subscription. The only limits are on Sage itself (a small number of free questions total) and AI plant-photo identification (a daily cap) — both call paid AI services behind the scenes, unlike everything else in the app. A promo code (entered under Help → Basic/Advanced mode) removes both limits.
- Bottom navigation tabs: Report (dashboard), Map, List, Water (irrigation), Audit, Help. Audit only appears once the user has turned on Advanced mode (Help → Basic/Advanced mode).
...
- Weather-aware reminders (skip/flag watering when rain is expected) are a separate toggle: Help → Weather-aware reminders. Unlike the other Advanced-mode features below, this one is available in Basic mode too, not something Advanced mode unlocks.
...
- Sun map, smart-irrigation integration (Tuya or Rachio), the companion planting/spacing audit, cost & water usage tracking, growth photo timelines, and watering history all require Advanced mode turned on (Help → Basic/Advanced mode) — nothing already entered is ever lost if the user switches back to Basic mode.
- Tuya/Rachio credentials and zone mapping are entered in Help → Irrigation (only visible in Advanced mode).
- Dropbox backup/export and CSV import/export live in Help → Data, and are unaffected by Basic/Advanced mode.
- The plant care home-screen widget can be reconfigured either by long-pressing it on the home screen, or from inside the app at Help → Plant care widget → Edit. It can show plants due for watering, pruning, fertilising, and/or feeding — pick which via the widget's settings.
