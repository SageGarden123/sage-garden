...
data class RainForecast(val maxProbabilityPercent: Int, val totalPrecipitationMm: Double, val minTempCelsius: Double?)
    ...
    suspend fun fetchTodayForecast(lat: Double, lng: Double): RainForecast? = withContext(Dispatchers.IO) {
        try {
            ...
            val url = "https://api.open-meteo.com/v1/forecast?latitude=$lat&longitude=$lng" +
                    "&daily=precipitation_probability_max,precipitation_sum,temperature_2m_min&timezone=auto&forecast_days=1"
            ...
            client.newCall(request).execute().use { response ->
                ...
                RainForecast(prob, precip, minTemp)
            }
