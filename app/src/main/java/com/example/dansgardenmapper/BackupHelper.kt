...
object BackupHelper {
    ...
    private suspend fun buildBackupPayload(
        ...
        db.sunZoneDao().getAllOnce().forEach { z ->
            val o = JSONObject()
            o.put("id", z.id); o.put("category", z.category); o.put("pointsJson", z.pointsJson)
            sunZonesArr.put(o)
        }
    ...
    private suspend fun applyBackupRoot(
        ...
        for (i in 0 until sunZonesArr.length()) {
            ...
            db.sunZoneDao().upsert(SunZoneEntity(id = o.getString("id"), category = o.getString("category"), pointsJson = o.getString("pointsJson")))
        }
