...
fun WidgetConfigScreen(initialConfig: WidgetConfig, onSave: (WidgetConfig) -> Unit) {
    ...
    var showPhotos by remember { mutableStateOf(initialConfig.showPhotos) }
    ...
    Column(Modifier.fillMaxSize().padding(20.dp)) {
        ...
        Row(verticalAlignment = Alignment.CenterVertically, modifier = Modifier.fillMaxWidth()) {
            Text("Show plant photos", fontSize = 13.sp, modifier = Modifier.weight(1f))
            Switch(checked = showPhotos, onCheckedChange = { showPhotos = it })
        }
        if (showPhotos) {
            Spacer(Modifier.height(4.dp))
            Text("Photos add a moment of loading time on refresh, especially for Dropbox-hosted ones.", fontSize = 11.sp, color = Color.Gray)
        }

        Spacer(Modifier.height(28.dp))
        Button(
            onClick = { onSave(WidgetConfig(intervalDays, maxPlants, showPhotos, lookaheadDays)) },
